...
exports.teamsAvailable = function(req, res) {
  ...
  var queryText =
    "SELECT tms.Team_ID, Abbreviation, Team_Name, c.Conference_Name, tul.User_ID, u.Username FROM Teams tms ";
  queryText +=
    "inner join Team_Conferences_Link tcl on tms.Team_ID = tcl.Team_ID ";
  queryText +=
    "inner join Conferences c on c.Conference_ID = tcl.Conference_ID ";
  queryText += "left join Team_Users_Link tul on tul.Team_ID = tms.Team_ID ";
  queryText += "left join Users u on tul.User_ID = u.User_ID ";
  queryText +=
    "WHERE tms.Team_ID Not in (Select tul2.Team_ID from Team_Users_Link tul2)";
  ...
  db.query(queryText, function(err, results, fields) {
    ...
    if (err) throw err;
    if (results.length > 0) {
      res.render("pages/user/teams/available", {
        userID: req.user.userID,
        username: req.user.username,
        roleID: req.user.roleID,
        teams: results,
...
exports.teamsRequest = function(req, res) {
  ...
  console.log(
    "req.session.passport.user.userID " + req.session.passport.user.userID
  );
  ...
  db.query(queryText, function(err, results, fields) {
    ...
    db.query(queryText2, function(err, results, fields) {
      ...
      if (results.length > 0) {
        res.render("pages/user/teams/available", {
          successMessage:
            "Your request has been submitted and an admin will review.",
          userID: req.user.userID,
          username: req.user.username,
          roleID: req.user.roleID,
          teams: results,
